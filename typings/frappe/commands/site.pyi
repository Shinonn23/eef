...
@click.option(
    ...
    help="Set the mariadb host for the user login scope if you don't want to use the current host as login "
...
def restore_backup(
    sql_file_path: str, site, db_root_username, db_root_password, verbose, install_app, admin_password, force
):  # -> None:
    ...
...
@pass_context
def reinstall(
    context: CliCtxObj, admin_password=..., db_root_username=..., db_root_password=..., yes=...
):  # -> None:
...
@pass_context
def migrate(context: CliCtxObj, skip_failing=..., skip_search_index=...):  # -> None:
...
@click.option(
    "--exclude", "-e", default="", type=str, help="Specify the DocTypes to not backup seperated by commas"
)
...
@click.argument("app")
@click.option(
    "--yes", "-y", help="To bypass confirmation prompt for uninstalling the app", is_flag=True, default=False
)
...
def handle_data(data: dict, format=...):  # -> None:
    ...
def add_new_user(
    email, first_name=..., last_name=..., user_type=..., send_welcome_email=..., password=..., role=...
):  # -> None:
    ...
