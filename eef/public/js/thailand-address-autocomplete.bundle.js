...
class ThailandGeography {
    ...
    _matchesSearch(nameEn, nameTh, searchTerm) {
        ...
        return (
            nameEn.toLowerCase().includes(term) || nameTh.includes(searchTerm)
        );
    }
    ...
    getDistrictsByProvince(provinceName) {
        ...
        const province = this.getAllProvinces().find(
            (p) =>
    ...
    getSubdistricts(provinceName, districtName = null) {
        ...
        let filteredData = this.data.filter(
            (item) => item.provinceCode === province.provinceCode,
        );

        // If district name is provided, filter by district
        if (districtName) {
            const district = this.getDistrictsByProvince(provinceName).find(
                (d) =>
                    d.districtNameTh === districtName ||
                    d.districtNameEn === districtName ||
                    d.districtNameEn.toLowerCase() ===
            ...
            filteredData = filteredData.filter(
                (item) => item.districtCode === district.districtCode,
            );
        }
    ...
    getPostalCodes(provinceName, districtName, subdistrictName) {
        ...
        const subdistrict = this.getSubdistricts(
            provinceName,
            districtName,
        ).find(
            (s) =>
                ...
                s.subdistrictNameEn.toLowerCase() ===
    ...
    search(searchTerm, type = "all", provinceName = null, districtName = null) {
        ...
        if ((type === "district" || type === "all") && provinceName) {
            results.districts = this.getDistrictsByProvince(
                provinceName,
            ).filter((d) =>
                this._matchesSearch(
                    d.districtNameEn,
        ...
        if (
            (type === "subdistrict" || type === "all") &&
            provinceName &&
            districtName
        ) {
            results.subdistricts = this.getSubdistricts(
                provinceName,
                districtName,
            ).filter((s) =>
                this._matchesSearch(
    ...
    searchDistricts(provinceName, searchTerm) {
        return (
            this.search(searchTerm, "district", provinceName).districts || []
        );
    }

    searchSubdistricts(provinceName, districtName, searchTerm) {
        return (
            this.search(searchTerm, "subdistrict", provinceName, districtName)
                .subdistricts || []
        );
    }
...
if (typeof window !== "undefined") {
    ...
    window.eef.geography = {
        ...
        getDistricts: (provinceName) =>
            thailandGeography.getDistrictsByProvince(provinceName),
        getSubdistricts: (provinceName, districtName = null) =>
            thailandGeography.getSubdistricts(provinceName, districtName),
        getByPostalCode: (postalCode) =>
            thailandGeography.getAddressByPostalCode(postalCode),
        getAddressInfo: (
            provinceName,
            districtName = null,
            subdistrictName = null,
        ) =>
            thailandGeography.getAddressInfo(
                provinceName,
                districtName,
                subdistrictName,
            ),
        ...
        smartSearch: (params) => {
            ...
            if (subdistrict && district) {
                const result = thailandGeography.getAddressInfo(
                    provinceName,
                    district,
                    subdistrict,
                );
                ...
                return {
                    type: "subdistricts",
                    results: thailandGeography.getSubdistricts(
                        provinceName,
                        district,
                    ),
                };
